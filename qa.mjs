...
const root = path.resolve(import.meta.dirname), publicDir = path.join(root, 'public'), buildDir = path.join(root, 'build');
...
function staticChecks() {
  ...
  walk(publicDir);
  assert(allPublic.length === 26, `Public artifact must contain exactly 26 files, found ${allPublic.length}`);
  ...
  results.static = { requiredFiles: required.length, publicFiles: allPublic.length, htmlPages: pages.length, sampleChecks: report.checks.length, cspCompatiblePages: pages.length, remoteAssets: 0, browserStorageCalls: 0, postRequests: 0, demoRunnerLocked: true };
}
async function waitForServer() { for (let attempt = 0; attempt < 40; attempt += 1) { try { const response = await fetch(`${base}/index.html`); if (response.ok) return; } catch {} await new Promise(resolve => setTimeout(resolve, 100)); } throw new Error('Local server did not become ready'); }
async function browserChecks() {
  const serverLog = fs.openSync(path.join(buildDir, 'qa-server.log'), 'w');
  const server = spawn('python3', ['-m', 'http.server', '4173', '--bind', '127.0.0.1', '--directory', publicDir], { stdio: ['ignore', serverLog, serverLog] });
  let browser;
  try {
    await waitForServer();
    ...
    assert(await page.locator('[data-workbench-panel="run"]').isVisible(), 'Run stage should be the initial focus');
    ...
    await page.selectOption('[data-filter="status"]', 'fail'); await waitForCount(page.locator('[data-report-rows] tr'), 4); await page.selectOption('[data-filter="status"]', '');
    await page.locator('[data-phase="after"]').click(); await waitForText(page.locator('[data-metric="pass"]'), '15');
    ...
    assert(await page.locator('[data-evidence-dock]').isVisible(), 'Selecting a row did not open the evidence dock');
    assert((await page.locator('[data-evidence-panel]').innerText()).toLowerCase().includes('after'), 'Evidence dock did not render selected after state');
    await page.locator('[data-close-evidence]').click();
    for (const stage of ['matrix', 'repair', 'report']) { await page.locator(`[data-workbench-stage="${stage}"]`).click(); assert(await page.locator(`[data-workbench-panel="${stage}"]`).isVisible(), `${stage} stage did not become visible`); assert(await page.locator(`[data-workbench-stage="${stage}"]`).getAttribute('aria-current') === 'step', `${stage} stage lacks current-step semantics`); }
    ...
    await page.locator('#stack').selectOption({ label: 'Lovable + Supabase' }); await page.locator('#environment').selectOption({ label: 'Staging available' }); await page.locator('#tables').fill('14'); await page.locator('#roles').fill('3'); await page.locator('#rpc').selectOption({ label: 'Yes' }); await page.locator('#storage').selectOption({ label: 'Yes' }); await page.locator('#concern').fill('Managers must not access another organization’s projects.'); await page.getByRole('button', { name: 'Prepare brief' }).click(); await page.locator('#contact-name').fill('Sample Founder'); await page.locator('#contact-email').fill('sample@invalid.test');
    const brief = await page.locator('[data-brief-output]').innerText();
    assert(brief.includes('Fictional Atlas CRM'), 'Scope brief missing application name'); assert(brief.includes('Sample Founder'), 'Scope brief did not update local contact name'); assert(brief.includes('has not been transmitted'), 'Scope brief missing non-transmission statement');
    await context.close();
    ...
    mobile.on('console', message => { if (message.type() === 'error') mobileErrors.push(message.text()); }); mobile.on('pageerror', error => mobileErrors.push(error.message));
    ...
    results.browser = { pagesLoaded: pages.length, titles, homeWorkbenchPreview: true, focusStages: ['scope', 'matrix', 'run', 'repair', 'report'], reportBefore: { total: 16, pass: 11, fail: 4, unresolved: 1 }, reportAfter: { total: 16, pass: 15, fail: 0, unresolved: 1 }, statusFilter: true, evidenceDock: true, localFileImport: true, scopeWizard: true, mobileMenu: true, mobileOverflowPages: 0, reducedMotion: true, consoleErrors: 0, externalRequests: 0 };
  } finally { if (browser) await browser.close(); server.kill('SIGTERM'); fs.closeSync(serverLog); }
}
try { staticChecks(); await browserChecks(); results.ok = true; fs.writeFileSync(path.join(buildDir, 'qa-results.json'), `${JSON.stringify(results, null, 2)}\n`); console.log(JSON.stringify(results, null, 2)); }
catch (error) { results.ok = false; results.error = error.stack || error.message; fs.writeFileSync(path.join(buildDir, 'qa-results.json'), `${JSON.stringify(results, null, 2)}\n`); console.error(JSON.stringify(results, null, 2)); process.exit(1); }
