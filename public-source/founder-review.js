(() => {
  ...
  const operatorStatus = document.querySelector('footer small');
  if (operatorStatus) operatorStatus.textContent = applicationsOpen
    ? `Service operator: ${config.businessName} · ${config.businessCountry} · ${config.contactEmail}`
    : 'Founder review applications are closed pending public operator and checkout setup.';
