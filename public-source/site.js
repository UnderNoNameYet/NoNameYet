(() => {
  ...
  footerLinks?.classList.add('site-more-links');
  ...
  document.querySelectorAll('.reason-switch button').forEach(button => button.addEventListener('click', () => {
    document.querySelectorAll('.reason-switch button').forEach(item => item.classList.remove('active')); button.classList.add('active');
    const data=reasons[button.dataset.reason]; document.querySelector('#reasonTitle').textContent=data.title; document.querySelector('#reasonCore').innerHTML=data.core; document.querySelector('#reasonCount').textContent=data.count;
  ...
  const updateScroll=()=>{const max=document.documentElement.scrollHeight-innerHeight;meter.style.transform=`scaleX(${max>0?scrollY/max:0})`;};
  ...
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches){const canvas=document.querySelector('.case-lab');canvas?.addEventListener('pointermove',event=>{if(stage.classList.contains('assembled'))return;const box=canvas.getBoundingClientRect();const x=(event.clientX-box.left)/box.width-.5;const y=(event.clientY-box.top)/box.height-.5;stage.style.setProperty('--mx',`${x*7}px`);stage.style.setProperty('--my',`${y*5}px`);});canvas?.addEventListener('pointerleave',()=>{stage.style.setProperty('--mx','0px');stage.style.setProperty('--my','0px');});}
})();
