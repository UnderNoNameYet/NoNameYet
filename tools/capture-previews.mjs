...
try {
  ...
  await page.screenshot({ path: path.join(out, 'tenantboundary-hero-desktop.png') });
  ...
  await page.screenshot({ path: path.join(out, 'tenantboundary-report-desktop.png') });
  await page.locator('[data-workbench-stage="matrix"]').click();
  await page.waitForTimeout(180);
  await page.screenshot({ path: path.join(out, 'tenantboundary-matrix-desktop.png') });
  await desktop.close();
  ...
  await mobilePage.screenshot({ path: path.join(out, 'tenantboundary-request-mobile.png'), fullPage: true });
  await mobilePage.goto(`${base}/index.html`, { waitUntil: 'networkidle' });
  await mobilePage.screenshot({ path: path.join(out, 'tenantboundary-mobile.png'), fullPage: true });
  await mobile.close();
  if (errors.length) throw new Error(`Preview browser errors: ${errors.join(' | ')}`);
  console.log(JSON.stringify({ files: ['tenantboundary-hero-desktop.png', 'tenantboundary-report-desktop.png', 'tenantboundary-matrix-desktop.png', 'tenantboundary-request-mobile.png', 'tenantboundary-mobile.png'] }, null, 2));
} finally {
