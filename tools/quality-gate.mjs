...
function run(name, command, args = [], options = {}) {
  ...
  const result = spawnSync(command, args, { cwd: root, encoding: 'utf8', maxBuffer: 24 * 1024 * 1024 });
  ...
  if (!ok) { failed = true; console.error(`\n[${name}] failed\n${result.stdout || ''}${result.stderr || ''}`); }
...
const scriptFiles = [];
function walkScripts(dir) { for (const entry of fs.readdirSync(dir, { withFileTypes: true })) { if (['node_modules', 'build', 'dist'].includes(entry.name)) continue; const full = path.join(dir, entry.name); if (entry.isDirectory()) walkScripts(full); else if (/\.(mjs|js)$/.test(entry.name)) scriptFiles.push(path.relative(root, full)); } }
walkScripts(root);
...
run('capture production previews', process.execPath, ['tools/capture-previews.mjs']);
run('record walkthrough', process.execPath, ['tools/record-walkthrough.mjs']);
...
function collectText(target) { const full = path.join(root, target); if (!fs.existsSync(full)) return []; const stat = fs.statSync(full); if (stat.isFile()) return [full]; return fs.readdirSync(full, { withFileTypes: true }).flatMap(entry => collectText(path.join(target, entry.name))); }
const secretHits = [];
