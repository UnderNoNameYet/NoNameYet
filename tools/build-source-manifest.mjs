...
const excluded = new Set([path.normalize(output), path.join(root, 'config/site.release.json')]);
...
const manifest = { generatedAt: new Date().toISOString(), product: 'TenantProof', version: '0.3.2', sourceOfTruth: 'AGENTS.md and handoff/README.md', deployRoot: 'public/', files: files.length, bytes: files.reduce((sum, file) => sum + file.bytes, 0), byRole: grouped, entries: files };
