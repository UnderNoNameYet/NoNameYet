...
add(current.repository?.tenantProofDeployed === true && /^[a-f0-9]{40}$/.test(current.repository?.mergeSha || '') && current.repository?.workflowConclusion === 'success' && current.repository?.deployedUrl === 'https://undernonameyet.github.io/NoNameYet/', 'Current state records verified GitHub Pages deployment');
