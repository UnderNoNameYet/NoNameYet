...
const required = [
  'AGENTS.md', 'agent.md', 'README.md', 'ARCHITECTURE.md', 'ROADMAP.md', 'SECURITY.md', 'CHANGELOG.md', 'CONTRIBUTING.md',
  '.github/pull_request_template.md',
  'handoff/README.md', 'handoff/HANDOFF_REPORT.md', 'handoff/PRODUCT_SPEC.md',
  'handoff/PAGE_AND_SECTION_SPECS.md', 'handoff/UX_DESIGN_SYSTEM.md',
  'handoff/TECHNICAL_ARCHITECTURE.md', 'handoff/FEATURE_AND_FUNCTION_INVENTORY.md',
  'handoff/REPORT_ENGINE_SPEC.md', 'handoff/SECURITY_PRIVACY_AND_SAFETY.md',
  'handoff/OPERATIONS_AND_INTAKE.md', 'handoff/COMMERCIAL_AND_PAYMENTS.md',
  'handoff/GTM_AND_VALIDATION.md', 'handoff/ROADMAP_V1_V2_V3.md',
  'handoff/DECISION_LOG.md', 'handoff/REJECTED_IDEAS_AND_LEARNINGS.md',
  'handoff/QUALITY_GATES.md', 'handoff/RELEASE_AND_REPOSITORY.md',
  'handoff/AI_CONTINUATION.md', 'handoff/CURRENT_STATE.json', 'handoff/ASSET_MANIFEST.md', 'handoff/GITHUB_PUBLICATION.md',
...
const markdownFiles = [
  ...
  ...walk(path.join(root, 'handoff')).filter(file => file.endsWith('.md')),
...
for (const file of markdownFiles) {
  ...
  const links = [...withoutFences.matchAll(/!?\[[^\]]*\]\(([^)]+)\)/g)].map(match => match[1].trim());
  for (const target of links) {
    ...
    add(destination.startsWith(root) && fs.existsSync(destination), `Internal link resolves: ${path.relative(root, file)} -> ${target}`);
  }
}
...
const dimensions = {
  '05-social-preview.png': [1200, 630],
  '06-icon-192.png': [192, 192],
  '07-icon-512.png': [512, 512]
};
for (const [name, expected] of Object.entries(dimensions)) {
  const file = path.join(root, 'handoff/assets', name);
  if (!fs.existsSync(file)) continue;
  const actual = pngDimensions(file);
  add(actual?.width === expected[0] && actual?.height === expected[1], `${name} dimensions are ${expected[0]}x${expected[1]}`);
}

const video = path.join(root, 'handoff/assets/walkthrough.webm');
if (fs.existsSync(video)) add(fs.statSync(video).size > 100_000, 'Walkthrough video is non-empty');
const handoffPdf = path.join(root, 'handoff/HANDOFF_REPORT.pdf');
if (fs.existsSync(handoffPdf)) add(fs.statSync(handoffPdf).size > 50_000, 'Handoff PDF is non-empty');

const sessionUrlPattern = /\{\{(?:notion|page|database|data-source|view|user|agent|file|thread|integration)-\d+\}\}/i;
for (const file of [...markdownFiles, path.join(root, 'handoff/CURRENT_STATE.json')]) {
  const source = fs.readFileSync(file, 'utf8');
  add(!sessionUrlPattern.test(source), `No session-only compressed URL in ${path.relative(root, file)}`);
}
